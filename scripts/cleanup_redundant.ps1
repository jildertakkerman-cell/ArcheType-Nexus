...
Get-ChildItem -Filter "*.html" | Where-Object { 
    ...
    # Remove individual fetchAndProcessCardData calls (after CardLoader.loadCards)
    $content = $content -replace '(?s)(CardLoader\.loadCards\(\{.*?\}\);\s*)(.*?)(?=\s*(?:</script>|function|\}))', $1$2 -replace 'fetchAndProcessCardData\([^;]+\);\s*', '' -replace '\s+$', ''
